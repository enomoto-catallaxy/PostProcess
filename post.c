...
//------------------------------------------------------------------------------   
{
    ...
    rc = sqrt(pow(cel->rc[0],2.0)+pow(cel->rc[1],2.0));//Distance from origin to centroid capsule[m] 
    ...
    //rc = sqrt(pow(cel->rc[0],2.0)+pow(cel->rc[1],2.0))/(cdo->D/2.0); //Non-dime
    rc = sqrt(pow(cel->rc[0],2.0)+pow(cel->rc[1],2.0)); //Non-dime

    //Get asymtotic time
    AT = (DX/(cdo->D/2.0) > rc) ? rt : 0.0;
    ...
    fprintf(fp,"%15.9e %15.9e %15.9e %15.9e %15.9e %15.9e %15.9e %15.9e \n",rt,T,cel->rc[2],cel->vc[2],sin(M_PI*rt/cdo->freq),rc/(cdo->D/2.0),DT, AT);
    fclose(fp);

    if (rc < dx && flag == false) {
      sprintf(filename,"./threshold/%s.dat",fname);
      fp = fopen(filename,"w"); if (fp == NULL) error(2);
      fprintf(fp,"%15.9e %15.9e %15.9e %15.9e\n",rt,T,rc/(cdo->D/2.0),dx);
      flag = true; 
    }
